...
urlpatterns = [
    ...
    path("password/email/", views.PasswordResetView.as_view(), name="password_reset"),  # 비밀번호 찾기 (이메일 보내기)
    path("auth/password/reset/b'<str:uidb64>'/<str:token>/", views.PasswordTokenCheckView.as_view(), name="password_reset_confirm"),  # 비밀번호 재설정 토큰 확인
    path("password/reset/", views.SetNewPasswordView.as_view(), name="password_reset_confirm"),  # 비밀번호 재설정
